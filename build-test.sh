...
#
#   ./build-test.sh parse    # layer + recipe parse only
#   ./build-test.sh fetch    # also fetch, validating every checksum
#   ./build-test.sh build    # also compile (needs a cross toolchain: hours)
#
...
docker run --rm \
    -v "$volume:/out" \
    -v "$root:/layers/meta-nerveshub:ro" \
    -e "STAGE=$stage" \
